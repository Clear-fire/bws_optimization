...
def main():
    parser = argparse.ArgumentParser(description='Caaa')
...
def train_net(data, args, save_dir):
    ...
    for i in range(nepochs):
        ...
        print(
            'Epoch {}: train loss {:.4f}, eval {:.4f}, dist {:.4f}, ineq max {:.8f}, ineq mean {:.8f}, eq max {:.8f}'.format(
                ...
                np.mean(epoch_stats['valid_ineq_mean']), np.mean(epoch_stats['valid_eq_max']) ))
...
class NNSolver(nn.Module):
    def __init__(self, data, args):
        ...
        layers = reduce(operator.add,
            [[nn.Linear(a,b), nn.BatchNorm1d(b), nn.ReLU(), nn.Dropout(p=0.10)] #p=0.2(O),p=0.1
        ...
        for layer in layers:
            if type(layer) == nn.Linear:
                nn.init.kaiming_normal_(layer.weight) #Standard initialization
