...
def main():
    ...
    for solver in solvers:
        ...
        print("avg time is:{} and parallel time is:{}".format(valid_time_total/100,valid_time_parallel))
        opt_results = get_opt_results(data, args, torch.tensor(Yvalid_opt).to(DEVICE),
                                       torch.tensor(Ytest_opt).to(DEVICE))
        opt_results.update(
            dict([('test_time', valid_time_parallel), ('valid_time', valid_time_parallel), ('train_time', 0),
                ('test_time_total', valid_time_total), ('valid_time_total', valid_time_total), ('train_time_total', 0)]))
...
def get_opt_results(data, args, Yvalid, Ytest, Yvalid_precorr=None, Ytest_precorr=None):
    ...
    results['valid_eq_num_viol_2'] = torch.sum(torch.abs(data.eq_resid(data.validX, Yvalid)) > 100 * eps_converge,
                                                   dim=1).detach().cpu().numpy()
    print("obj is:{}, ineq_max is:{}, ineq_max is:{}".format(np.mean(results['valid_eval']),np.mean(results['valid_ineq_max']),np.mean(results['valid_eq_max'])))
    ...
    print("obj is:{}, ineq_max is:{}, ineq_max is:{}".format(np.mean(results['test_eval']),np.mean(results['test_ineq_max']),np.mean(results['test_eq_max'])))
